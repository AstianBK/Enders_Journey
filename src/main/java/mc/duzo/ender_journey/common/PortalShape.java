...
public class PortalShape {
    ...
    private int getDistanceUntilTop(BlockPos.MutableBlockPos mutablePos) {
        for (int i = 0; i < 21; ++i) {
            ...
            for (int j = 0; j < this.width; ++j) {
                ...
                if (blockState.is(BKBlocks.PORTAL.get())) {
    ...
    private static boolean isEmpty(BlockState state) {
        return state.isAir() || state.is(Blocks.WATER) || state.is(BKBlocks.PORTAL.get());
    }
    ...
    public void createPortalBlocks() {
        BlockState blockState = BKBlocks.PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, this.axis);
